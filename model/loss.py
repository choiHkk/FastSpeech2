...
class FastSpeech2Loss(nn.Module):
    ...
    def forward(self, inputs, predictions, step):
        ...
        if step > self.var_start_steps:
            ...
            pitch_loss = torch.zeros(1).to(postnet_mel_loss.device)
            energy_loss = torch.zeros(1).to(postnet_mel_loss.device)
            duration_loss = torch.zeros(1).to(postnet_mel_loss.device)
