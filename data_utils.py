...
class AudioTextCollate(object):
    def __call__(self, batch):
        # speaker_id, text, mel, pitch, energy, attn_prior
...
class AudioTextDataset(Dataset):
    ...
    def get_values(self, line):
        ...
        values[5] = torch.from_numpy(values[5]).float()
        return values
...
class AudioTextProcessor(object):
    ...
    def process_utterance(self, line):
        ...
        energy = self.remove_outlier(energy[: duration])
